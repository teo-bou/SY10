...
def generate_village(carte, number):
    ...
    for i in range(number):
        ...
        b = random.randint(0,2000) + a
        c = random.randint(0, 2000) + b
        ift_pop = NFT(a,b,c,1,"habitants village "+str(i))
        x,y = random.randint(0, int(carte.x_max)), random.randint(0, int(carte.y_max))
        village = Village(carte, x,y,ift_pop, ressenti.v(random.random(), random.random()), {"hopital" : random.randint(0,int(map_range(b,0,2000, 0, 5))), "ecole":  int(map_range(b,0,2000, 0, 20)), "gouvernement": int(map_range(b,0,2000, 0, 10))})
